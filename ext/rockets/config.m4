...
if test "$PHP_ROCKETS" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $ROCKETS_DIR/$PHP_LIBDIR, ROCKETS_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_ROCKETSLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong rockets lib version or lib not found])
  dnl ],[
  dnl   -L$ROCKETS_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(ROCKETS_SHARED_LIBADD)

  PHP_NEW_EXTENSION(rockets, rockets.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
